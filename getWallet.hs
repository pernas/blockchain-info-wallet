...
splitPayloadSalt :: ByteString -> (ByteString, ByteString)
splitPayloadSalt payloadWithSalt = ByteString.splitAt 16 payloadWithSalt

decodeB64 :: Text -> Maybe ByteString
decodeB64 base64String =
  either
    (const Nothing)
    return
...
decrypt iterations password payload64 = do
  ...
  let (salt, payload) = splitPayloadSalt payloadWithSalt
...
main = do
    ...
    case valM of
        Nothing  -> error "Colud not get anything"
